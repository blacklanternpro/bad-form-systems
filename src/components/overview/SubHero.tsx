...
export function SubHero() {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex min-h-[calc(100dvh-5.25rem)] w-full max-w-[36rem] flex-col items-start justify-end px-4 py-8 sm:px-7 lg:px-10 xl:pl-14">
        <h2
          id="subhero-heading"
          className="type-hero type-on-still mb-4 max-w-[16ch] text-[clamp(2.15rem,5.5vw+0.9rem,3.8rem)]"
        >
          {subhero.title}
        </h2>
        {ledger ? (
          <p className="type-docket type-on-still-docket max-w-[28ch] border-y border-brand-ply py-3 text-base">
            {ledger.title}.
          </p>
        ) : null}
      </div>
